describe('测试Todo App的添加功能',function () {
    it('添加一项待办事项', function () {
        ...
        //测试添加功能
        cy.get('#app > div > form > input[type="text"]').type('study mathmatic')
        cy.get('#app > div > form > button').click()
        cy.get('#app > div > form > input[type="text"]').type('running')
        cy.get('#app > div > form > button').click()
        //测试删除功能
        cy.get('#app > div > form > input[type="text"]').type('sleep')
        cy.get('#app > div > form > button').click()
        ...
        cy.get('#app > div > table > tbody > tr:nth-child(2) > td:nth-child(1) > form > input').type('learning')
        cy.get('#app > div > table > tbody > tr:nth-child(2) > td:nth-child(2) > button:nth-child(1)').click()
    })
})